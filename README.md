...
2. Automagic is also available as a EEGLAB pluging and you can use it to preprocess data loaded by EEGLAB gui. See [Automagic as EEGLAB plugin](#3-automagic-as-eeglab-plugin)
...
1. **`preprocessing/`**
 This folder contains all relevant files of preprocessing step. The folder is standalone and can be used independent from the entire application. The main function to be called is `pre_process.m` which as argument needs the EEG data structure loaded by `pop_fileio.m` function (or a similar function) of **EEGLab** and the preprocessing parameters (See documations, ie. `pre_process.m` to learn about the second argument). For more information on how to run the code without installer please see  [How to run the app from the code](#4-how-to-run-the-application-from-the-code).
...
2. **`gui/`**
 ...
 2. `rating\_gui.m` is the gui that is accessed within the `main\_gui.m` and is used to rate subjects and files. You must not use this.
...
4. **`eeglab_plugin/`**
 This folder contains necessary files to integrate **Automagic** as an **EEGLab** plugin. There are corresponding `pop_` functions and equivalent functions of `src/` for the plugin. The structure is very similar to `src/`.
...
1. Download the [*eeglab*](https://sccn.ucsd.edu/eeglab/downloadtoolbox.php) library and put them in the `automagic/matlab_scripts` folder.
